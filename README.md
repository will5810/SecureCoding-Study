...
-> 공격자는 특정 파라미터를 조작해 목적을 달성한다.
   공격자는 정상적 인증을 과정을 수행한후 시스템을 접근하고 
    url,파라미터 조작후 목적달성
 
 취약점 원인
 1. 파라미터 url 조작
 2. 허가되지 않은 작업 요청
 3. 인증 & 인가에 대한 유효성 검증 부재
 
 ### -실습-
 
 1. 관리자 권한으로 user를 생성하고
 2. 그 파라미터를 paros로 trap 잡고 복사후
 3. 관리자 권한으로 user 삭제
 4. 일반사용자로 로그인하면 조회 밖에 안되는데
 5. paros로 trap잡고 복사한 파라미터를 붙히면 일반사용자가
    user를 생성할수 있다.

 ###방어
 1. 테스트 컨트롤러 클래스 부분에 보면
  기능별로 허가된 사용자인지 확인한 뒤 요청이 수행되도록 코드를 수행
 2. 파라미터를 발생시킨 사용자의 권한이 정당한지 체크
   && "admin".equals(session.getAttribute("userId"))
